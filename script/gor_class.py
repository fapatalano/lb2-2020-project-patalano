...
class Gor:
    ...
    def __init__(self, training_path, dssp_path, test_path, cv):
        self.ss = ['H', 'E', 'C', 'R']
        self.residue = ['A', 'R', 'N', 'D', 'C', 'Q', 'E', 'G', 'H',
        ...
        if cv:
            ...
            for k in range(len(k_fold.keys())):
                self.training_id = []
                [self.training_id.extend(values)
                 for key, values in k_fold.items() if key != k]
            ...
            acc = prediction[0]
            classification_report = prediction[2]
            multiclass_conf_mat = multilabel_confusion_matrix(
                self.y_true, self.y_pred, labels=['H', 'E', 'C'])
    ...
    @staticmethod
    def to_dict(file):
        dict = file.groupby(level=0).apply(
            lambda df: df.xs(df.name).to_numpy()).to_dict()
        return dict
    ...
    def get_y_true(path, ids):
        ...
        for id in ids:
            ...
            [y_true.append('C') if x == '-' else y_true.append(x)
             for x in dssp]
        return y_true
...
# convert to a static method
    def to_df(self, gor_model):
        d = self.w // 2
        window = range(-d, d + 1)
        model_df = pd.DataFrame(gor_model, pd.MultiIndex.from_product(
            [self.ss, window], names=['SS', 'PW']), self.residue)
        return model_df
    ...
    def build_information_matrix(self):
        ...
        for index in information_matrix.index:
            for column in information_matrix:
                ...
                marginal_residue = self.gor_model.loc[(
                    'R', windows_position), residue]
                if s_structure != 'R':
                    marginal_ss = self.ss_dict[s_structure]
                    information_matrix.loc[(s_structure, windows_position), residue] = math.log2(
                        joint_probability_r_ss / (marginal_ss * marginal_residue))
            ...
            with open(self.test_path + id + '.prf') as file:
                ...
                for i in range(0, (len(profile) - self.w + 1)):
                    ...
                    if np.sum(profile[i:i + self.w]) == 0:
                        ...
                        information_content['H'] = np.sum(np.multiply(
                            self.information_dict['H'], profile[i:i + self.w]))
                        information_content['E'] = np.sum(np.multiply(
                            self.information_dict['E'], profile[i:i + self.w]))
                        information_content['C'] = np.sum(np.multiply(
                            self.information_dict['C'], profile[i:i + self.w]))
                        predicted_ss = max(
                            information_content, key=information_content.get)
    ...
    def performance(self, f=False):
        ...
        if f == True:
            mcc = Gor.get_mcc(self.y_true, self.y_pred,
                              True)  # one structure vs all
        else:
            mcc = Gor.get_mcc(self.y_true, self.y_pred)
        class_report = classification_report(self.y_true, self.y_pred, labels=[
                                             'H', 'E', 'C'], output_dict=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("training", action='store',
                        help="path to the training test set directory")
    ...
    parser.add_argument("--cv", action='store', default=None,
                        help="Turn on the cross validation")
